...
float noise3(vec3 x) {
    ...
    vec2 e = vec2(0.0, 1.0);
    return mix(
        mix(mix(hash31(i + e.xxx), hash31(i + e.yxx), f.x),
            mix(hash31(i + e.xyx), hash31(i + e.yyx), f.x), f.y),
        mix(mix(hash31(i + e.xxy), hash31(i + e.yxy), f.x),
            mix(hash31(i + e.xyy), hash31(i + e.yyy), f.x), f.y), f.z
    );
}
...
void main() {
    ...
    vec3 col = mix(horizon, zenith, sqrt(max(ray.y, 0.0)));
    ...
    if (night > 0.0 && ray.y > -0.05) {
        ...
        if (auroraFade > 0.0 && moonDiscMask < 1.0) {
            ...
            float fakeTime = ubo.sunDir.x * 4.0 + ubo.sunDir.z * 2.0;
            aUv += fakeTime * 0.15;
            ...
            float streaks = fbm(vec2(ray.x * 15.0, ray.y * 30.0 + fakeTime));
        ...
        if (moonCos > 0.999) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                surface += (-0.30 * floorDark + 0.16 * rim) * hasCrater * w;
            }
