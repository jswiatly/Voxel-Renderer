...
class Renderer {
    public:
        void init(VulkanContext& ctx, Window& window, Swapchain& swapchain, Pipeline& pipeline, std::vector<Mesh>& chunks, ImGuiLayer& imgui);
        void cleanup();
        void drawFrame(const UniformBufferObject& ubo, const glm::vec4& clearColor);
        void setRenderDistance(float d) { m_renderDistance = d; }
